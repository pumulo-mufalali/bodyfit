...
export interface Achievement {
  ...
  title: string;
  description: string;
  icon: string; // Icon name (e.g., 'Trophy', 'Star', etc.)
  criteria: {
    type: 'duration' | 'count' | 'streak' | 'total_duration';
    value: number; // e.g., 30 for 30 minutes
  };
  achieved: boolean;
  achievedDate: string | null; // ISO date string or null
...
function checkAchievementCriteria(
...
): { achieved: boolean; progress: number; achievedDate: string | null } {
  ...
  switch (criteria.type) {
    ...
    case 'duration': {
      ...
      const matchingWorkout = sortedLogs.find(
        log => log.durationMinutes >= criteria.value
      );
...
export async function getUserAchievements(userId: string, workoutLogs: WorkoutLog[]): Promise<Achievement[]> {
  const achievementsRef = collection(db, 'users', userId, 'achievements');
  const snapshot = await getDocs(achievementsRef);
  const storedAchievements = new Map(
    snapshot.docs.map(doc => [doc.id, { id: doc.id, ...doc.data() } as Achievement])
  );
